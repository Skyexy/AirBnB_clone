	...
	def default(self, line):
		...
		try:
			...
			if model_com == "all()":
				...
				mod = re.findall(r'[^"^,\s^\)]+', model_com)
				modict = re.findall(r'{[{\w\s\"\:\'},]+', model_com)
				modid = mod[1]
				if modid:
					try:
						if modict:
							dic = modict[0]
							models.storage.update_dict(model_name, modid, dic)
							models.storage.save()
						else:
							att = mod[2]
							value = mod[3]
							if att == None:
								...
								models.storage.update(model_name, modid, att, value)
								models.storage.save()
					except Exception as e:
						print(e)
				else:
...
if __name__ == '__main__':
	HBNBCommand().cmdloop()
>>>>>>> 7754c47604659843d7218f1cda61bf303542a74b
